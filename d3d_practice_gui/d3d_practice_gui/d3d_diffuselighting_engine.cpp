...
DiffuseLightingEngine::DiffuseLightingEngine() :
	m_swapchain(nullptr),
	m_device(nullptr),
	...
	m_diffuse_color(0.f, 0.f, 1.f, 1.f),
...
void DiffuseLightingEngine::Init(HWND hwnd) {
	InitD3D(hwnd);
	InitD3DBuffer();
	InitD3DPipeline();
	InitD3DTexture();
...
void DiffuseLightingEngine::InitD3D(HWND hwnd) {
...
void DiffuseLightingEngine::CleanD3D() {
	if (m_texture) {
		m_texture->Release();
		m_texture = nullptr;
	}
...
void DiffuseLightingEngine::InitD3DPipeline() {
	ID3D10Blob *vertex_blob = nullptr, *pixel_blob = nullptr;
	D3DX11CompileFromFile(L"diffuselighting.shader", 0, 0, "VShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, 0, &vertex_blob, 0, 0);
	D3DX11CompileFromFile(L"diffuselighting.shader", 0, 0, "PShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, 0, &pixel_blob, 0, 0);
...
void DiffuseLightingEngine::InitD3DBuffer() {
	if (m_device) {
		{
			VERTEX3 *vertex = new VERTEX3[3];
			vertex[0].position = D3DXVECTOR3(-1.f, -1.f, 0.f);
			vertex[0].texture = D3DXVECTOR2(0.f, 1.f);
			vertex[0].normal = D3DXVECTOR3(0.f, 0.f, -1.f);
			vertex[1].position = D3DXVECTOR3(0.f, 1.f, 0.f);
			vertex[1].texture = D3DXVECTOR2(0.5f, 0.f);
			vertex[1].normal = D3DXVECTOR3(0.f, 0.f, -1.f);
			vertex[2].position = D3DXVECTOR3(1.f, -1.f, 0.f);
			vertex[2].texture = D3DXVECTOR2(1.f, 1.f);
			vertex[2].normal = D3DXVECTOR3(0.f, 0.f, -1.f);

			D3D11_BUFFER_DESC buf_desc;
			ZeroMemory(&buf_desc, sizeof(D3D11_BUFFER_DESC));

			buf_desc.Usage = D3D11_USAGE_DYNAMIC;
			buf_desc.ByteWidth = sizeof(VERTEX3) * 3;  // For drawgin triangle currently.
...
bool DiffuseLightingEngine::RenderFrame(const D3DXMATRIX &view_matrix) {
	if (m_device_context) {
...
void DiffuseLightingEngine::RenderTriangle() {
	if (m_device_context) {
