...
class Simple3DModelEngine : public ID3DEngine {
public:
	Simple3DModelEngine();
	virtual ~Simple3DModelEngine();

public:
	void Init(HWND hwnd) override;
	bool RenderFrame(const D3DXMATRIX &view_matrix) override;
	void SetRotationDegree(const float degree) override;
...
private:
	...
	std::vector<std::vector<float>> m_3d_model_vertexs;
};